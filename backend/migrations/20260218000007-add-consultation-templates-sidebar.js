...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    try {
      ...
      console.log('sidebar_menu_configs table does not exist, skipping');
      return;
    }

    // Check if item already exists
    const existing = await queryInterface.sequelize.query(
      "SELECT item_key FROM sidebar_menu_configs WHERE item_key = 'consultation-templates'",
      { type: Sequelize.QueryTypes.SELECT }
    );

    if (existing.length > 0) {
      console.log('consultation-templates sidebar item already exists, skipping');
      return;
    }
    ...
    await queryInterface.bulkInsert('sidebar_menu_configs', [{
      id: uuidv4(),
      item_key: 'consultation-templates',
      section: 'main',
    ...
    console.log('Added consultation-templates sidebar item');
  },
