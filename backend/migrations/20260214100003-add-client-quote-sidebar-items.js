...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    const existing = await queryInterface.sequelize.query(
      "SELECT item_key FROM sidebar_menu_configs WHERE item_key IN ('clients', 'quotes')",
    ...
    if (!existingKeys.includes('quotes')) {
      items.push({
        id: uuidv4(),
        item_key: 'quotes',
        section: 'main',
    ...
    if (items.length > 0) {
      ...
      console.log(`✅ Added ${items.length} sidebar items (clients, quotes)`);
    }
  },

  async down(queryInterface) {
    await queryInterface.bulkDelete('sidebar_menu_configs', {
      item_key: ['clients', 'quotes']
    });
  }
};
