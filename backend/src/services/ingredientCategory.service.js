 ...
 *
 * Business logic for ingredient category management with RBAC and audit logging.
 */
...
async function getCategories(user, filters = {}, requestMetadata = {}) {
  try {
    ...
    await auditService.log({
      ...
      action: 'READ',
      resource_type: 'ingredient_categories',
      resource_id: null,
...
async function getCategoryById(id, user, requestMetadata = {}) {
  try {
    ...
    if (!category) {
      const error = new Error('Ingredient category not found');
...
async function createCategory(categoryData, user, requestMetadata = {}) {
  try {
    ...
    const maxOrder = await IngredientCategory.max('display_order') || 0;
...
/**
 * Delete an ingredient category (soft delete)
 *
 * @param {string} id - Category UUID
 * @param {Object} user - Authenticated user
 * @param {Object} requestMetadata - Request metadata
...
async function deleteCategory(id, user, requestMetadata = {}) {
  try {
    ...
    if (ingredientCount > 0) {
      const error = new Error(`Cannot delete category with ${ingredientCount} ingredient(s). Move or delete ingredients first.`);
    ...
    return { success: true, message: 'Ingredient category deleted successfully' };
  } catch (error) {
    if (!error.statusCode) error.statusCode = 500;
    throw error;
  }
 ...
 */
async function getCategoryStats(user) {
  try {
    const stats = await IngredientCategory.findAll({
      ...
      attributes: [
        ...
        'icon',
        'color',
        [db.sequelize.fn('COUNT', db.sequelize.col('ingredients.id')), 'ingredient_count']
      ],
      include: [
        {
          model: db.Ingredient,
