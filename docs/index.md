# Project Documentation Index

### Project Overview

- **Type:** Multi-part with 2 parts (frontend, backend)
- **Primary Language:** JavaScript
- **Architecture:** Client-Server / SPA

### Quick Reference

#### Backend (backend)
- **Type:** backend
- **Tech Stack:** Express, Node.js, Sequelize
- **Root:** `/backend`

#### Frontend (frontend)
- **Type:** web
- **Tech Stack:** React, Vite
- **Root:** `/frontend`

### Generated Documentation

- [Project Overview](./project-overview.md)
- [Source Tree Analysis](./source-tree-analysis.md)
- [Development Guide](./development-guide.md)
- [Integration Architecture](./integration-architecture.md)
- [Architecture - Backend](./architecture-backend.md) _(To be generated)_
- [Architecture - Frontend](./architecture-frontend.md) _(To be generated)_
- [API Contracts](./api-contracts-backend.md) _(To be generated)_
- [Data Models](./data-models.md) _(To be generated)_

### Documentation Utilisateur

- [Guide Utilisateur (FR)](./GUIDE_UTILISATEUR.md) - Documentation complète en français

### Existing Documentation

- [Feature Plan](../plan/feature-nutrivault-complete-system-1.md)
- [Specifications](../SPECIFICATIONS.md)

### Getting Started

See [Development Guide](./development-guide.md) for setup and running instructions.
